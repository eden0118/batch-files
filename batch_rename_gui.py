...
iOS-inspired dark theme with multilingual support
"""
...
from styles import (
    THEMES, WINDOW_CONFIG, SPACING, BUTTON_STYLES,
    DEFAULT_THEME, get_widget_config
)
...
class BatchRenameApp:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.lang = LANGUAGES["en"]  # Always use English
        ...
        # 視窗尺寸限制
        # =====================================================================
        # 最小寬度: 900px (確保左右欄各 450px 以上)
        # 最小高度: 600px (確保內容區域有足夠高度)
        MIN_WINDOW_WIDTH = 900
        MIN_WINDOW_HEIGHT = 600
        self.root.minsize(MIN_WINDOW_WIDTH, MIN_WINDOW_HEIGHT)

        # Initialize S2T converter
        ...
        self.rename_mode: str = "files"  # "files" or "both"
    ...
    def apply_theme(self) -> None:
        ...
        for item in self.text_widgets.values():
            # Skip items that are not (widget, widget_type) tuples
            if isinstance(item, tuple) and len(item) == 2 and isinstance(item[1], str):
                widget, widget_type = item
                self._apply_style(widget, widget_type)

    def _apply_style(self, widget: tk.Widget, widget_type: str) -> None:
        """Apply style configuration to a widget"""
        try:
            config = get_widget_config(self.theme, widget_type)
            if config:
                # Convert flat to tk.FLAT
                if "relief" in config and config["relief"] == "flat":
    ...
    def setup_ui(self) -> None:
        """
        Setup user interface with responsive two-column layout:
        - Left Column (50%): Control Panel (Steps 01-04 + Preview)
        - Right Column (50%): Terminal Log Display
        ...
        self.select_btn = tk.Button(step01, text=self.lang["select_dir"],
                                    command=self.select_directory, font=self.theme["font_normal"],
...
# =====================================================================
        ...
        self.preview_btn = tk.Button(button_frame, text=self.lang["show_preview"],
                                     command=self.on_show_preview, font=self.theme["font_normal"])
        ...
        self.execute_btn = tk.Button(button_frame, text=self.lang["confirm_rename"],
                                     command=self.on_confirm_execute, font=self.theme["font_header"])
